...
//************************ PIN DEFINITIONS **************************************
static const byte LCD_PIN =  D0;     // gate pin to control LCD was D3
static const byte DST_PIN =  D5;     // gate pin to control distance sensor was D4
...
unsigned long turnOnDelay = 500; // wait to turn on LCD
unsigned long turnOffDelay = 5000; // turn off LED after this time
unsigned long turnOnLcdDelay = 500; // wait to turn on LED
unsigned long turnOffLcdDelay = 5000; // turn off LED after this time
...
//const char* IpAddr;
const char* Version = "NewBuild_TSTA";
#ifdef Wemos
...
#define ADC_ADJUST 175.33  // Using a 220K resistor, 4.02 = 4.02 = exact at 4.15 V
//*************************** END ADC VOLTAGE ******************************
...
const unsigned long sleeptime = 40000; // milliseconds
int numReadings = 2;
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    */
    if (client.connect("TbOfficeClient")) {
      Serial.println(F("connected to TbOfficeClient"));
      client.subscribe("TbOfficeClient/LcdState");
      client.subscribe("TbOfficeClient/reset_ESP");
    } else {
...
void serialPrintVcc(void)
{
  ...
  Serial.print(VCC); Serial.println(F(" Vdc\t"));
...
void publishSleepTime(void) {
  char sleepTime[12];
  client.publish("/TbOfficeClient/sleepTime", itoa(sleeptime/1000, sleepTime, 10));
}
...
char* makeTopic(char* value)
 ...
 int pubTopic(char Topic[], char* payload)
 {
  char topic[32];
  strcpy(topic, Client);
  strcat(topic, Topic);
...
void publishStaInfo(void)
{
  ...
  //publishTopic(ahostname, Version);
  //publishTopic(Version);
  char Topic[32];
  //strcpy(Topic, ahostname);
  ...
  //strcat(Topic, "/");
  //char Topic = strcat(hostname, Version);
  //strcat(Topic, "/")
  //client.publish("/TbOfficeClient/Version", Version);

  //pubTopic("Version", Version);
...
void publishBme(void)
{
  ...
  client.publish("/TbOfficeClient/temperature", bmeTemperatureF);
  client.publish("/TbOfficeClient/humidity", bmeHumidity);
  client.publish("/TbOfficeClient/pressure", bmePressure);
...
void publishNTP(void)
{
  ...
  // uptime
  strcpy(Uptime,Client);
  strcat(Uptime, "upTime");
  
  // Current time
  strcpy(CurrTime,Client);
  strcat(CurrTime, "currTime");
  
  // Boot Time
  strcpy(BootTime,Client);
  strcat(BootTime, "bootTime");
  
  //  Publish
  client.publish(Uptime,upTime.c_str()); 
  client.publish(CurrTime,currDate.c_str()); 
  client.publish(BootTime,bootDate.c_str()); 
...
void publishHeap(void)
{
  ...
  client.publish("TbOfficeClient/freeHeap", freeHeap);
...
void publishResetReason(void)
{
  ...
  client.publish("TbOfficeClient/lastResetReason", lastResetReason.c_str());
}
...
//#endif
  ...
  client.publish("/TbOfficeClient/Out/temperature", outTmpFString);
...
unsigned int checkDistance()
{
  ...
  /*
   * 11/17 - move lcd on millis stuff in here
   */
  else {
    Serial.print(F("sonar distance = "));
    Serial.println(Tmp);
    Serial.println(F("In range, LCD is ON"));
...
void checkChargerStatus(void)
{
  ...
  /*
    String pwr_State = "No Power to Charger";
    String chg_State = "Not Charging";
    String dne_State = "Still Charging";
  */

  char* pwr_State = "No Power to Charger";
  //char* chg_State = "Not Charging";
  char* dne_State = "Still Charging";

  // FIXME! Need to put in case where PWR_PIN == LOW AND CHRG_PIN == HIGH, means batt low
  // charger not powered

  if (digitalRead(PWR_PIN) == LOW)
  {
    //pwr_State = "No Power to Charger";
    strcpy(pwr_State, "Charging");

  }
  else {
    //pwr_State = "Charger is Powered";
    strcpy(pwr_State, "Not Charging");
  }
  ...
  {
    //dne_State = "Charged";
    strcpy(dne_State, "Charged");
  }
  else {
    //dne_State = "NOT Charged";
    strcpy(dne_State, "Not Charged");
  }

  client.publish("TbOfficeClient/pwrState", pwr_State);
  //client.publish("TbOfficeClient/chgState", chg_State);
  client.publish("TbOfficeClient/dneState", dne_State);

  //client.publish("TbOfficeClient/pwrState", pwr_State.c_str());
  //client.publish("TbOfficeClient/pwrState", "Power State");
  //client.publish("TbOfficeClient/chgState", chg_State.c_str());
  //client.publish("TbOfficeClient/dneState", dne_State.c_str());
}
 ...
 if (lcdState) {
  ...
  if (awake == true)
  {
    ...
    if (!client.loop())
      ...
      client.connect("TbOfficeClient");
    Serial.println(F("done with client.connect TB OFFICE"));
    Serial.println("Free Heap      = " + String(ESP.getFreeHeap(), DEC));
